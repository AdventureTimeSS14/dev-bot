...
async def get_pull_requests_awaiting_changes(repository):
    """Получает информацию о пулл-реквестах, требующих изменений, и возвращает её как список словарей."""
    try:
        g = Github(GITHUB)
        repo = g.get_repo(repository)

        # Получение списка пулл-реквестов, которые требуют изменений
        pulls = repo.get_pulls(state='open', sort='created', base='master')
        
        pull_requests_list = []
        for pr in pulls:
            # Проверяем наличие метки "Status: Awaiting Changes"
            if any(label.name == "Status: Awaiting Changes" for label in pr.labels):
                pull_requests_list.append({
                    "title": pr.title,
                    "url": pr.html_url,
                    "author": pr.user.login,
...
async def achang(ctx, repo_key: str):
    """Получает пулл-реквесты для указанного репозитория, требующие изменений."""
    # Проверка, имеет ли пользователь хотя бы одну разрешенную роль
    if any(role.id in whitelist_role for role in ctx.author.roles):
        if repo_key not in repositories:
            await ctx.send("Пожалуйста, укажите корректный репозиторий: n или o.")
        ...
        repository_name = f"{author}/{repositories[repo_key]}"
        pull_requests_list = await get_pull_requests_awaiting_changes(repository_name)
        ...
        # Делим список пулл-реквестов на несколько embed, чтобы не превышать ограничение по количеству полей
        embed_list = []
        current_embed = discord.Embed(title=f"Список пулл-реквестов, требующих изменений. \nРепозиторий: {repository_name}", color=discord.Color.dark_gold())
        ...
        for i, pr in enumerate(pull_requests_list):
            ...
            requested_by = ', '.join(pr['requested_by']) if pr['requested_by'] else 'Нет запрашиваемых рецензентов'  # Преобразуем список в строку
            current_embed.add_field(name=pr['title'], value=f"Автор: {pr['author']}\nЧьё ревью запрашено: {requested_by}\nСсылка: {pr['url']}", inline=False)

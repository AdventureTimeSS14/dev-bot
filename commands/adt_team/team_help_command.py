...
async def team_help(ctx):
    ...
    embed.add_field(
        name="P.S ПРИ НАПИСАНИИ РОЛЕЙ, КОТОРЫЕ СОСТОЯТ ИЗ 2-Х И БОЛЕЕ СЛОВ, ОБЯЗАТЕЛЬНО СТАВИМ ДВОЙНЫЕ КАВЫЧКИ И ПИШЕМ РОЛЬ В НИХ. АНАЛОЛГИЧНО И ДЛЯ НАПИСАНИЯ ПРИЧИНЫ. УЧИТЫВАЙТЕ МОЖНО ВПИСЫВАТЬ ПИНГ, ID ИЛИ НИКНЕЙМ ПОЛЬЗОВАТЕЛЯ ", 
        value="", 
        inline=True
    )
    embed.set_author(

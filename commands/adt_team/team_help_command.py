...
async def team_help(ctx):
    ...
    embed.add_field(
        ...
        value=f"```Закрывает и убирает пользователю роль отпуска и отправляет сообщение в {channel_get}.\nПример: &end_vacation @Darkiich```", 
        inline=False
    )
    embed.add_field(
    ...
    embed.add_field(
        name="P.S ПРИ НАПИСАНИИ РОЛЕЙ, КОТОРЫЕ СОСТОЯТ ИЗ 2-Х И БОЛЕЕ СЛОВ, ОБЯЗАТЕЛЬНО СТАВИМ ДВОЙНЫЕ КАВЫЧКИ И ПИШЕМ РОЛЬ В НИХ. АНАЛОЛГИЧНО И ДЛЯ НАПИСАНИЯ ПРИЧИНЫ.\nУЧИТЫВАЙТЕ МОЖНО ВПИСЫВАТЬ ПИНГ, ID ИЛИ НИКНЕЙМ ПОЛЬЗОВАТЕЛЯ\nС РОЛЬЮ ТАКЖЕ, МОЖНО ВПИСАТЬ ИМЯ РОЛИ, ID РОЛИ ИЛИ ПИНГ РОЛИ", 
        value="", 
        inline=True
    )
    embed.set_author(
